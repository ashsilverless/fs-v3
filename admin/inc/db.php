<?php
  ini_set("session.cache_expire", 600);  // 600 mins = 10 hours
  ini_set("session.gc_maxlifetime", 36000);  // 36,000 seconds = 10 hours
  ini_set("session.cookie_lifetime", 36000);  // 36,000 seconds = 10 hours
  ini_set('session.gc_probability',100);    // 1% of the time
  ini_set('session.gc_divisor',1);    // 1% of the time
...
function gimme_fund_shares($sedol){
	settype($sedol, "string");
	...
	try {
	  ...
	  $conn = new PDO("mysql:host=$host; dbname=$db", $user, $pass);
		  ...
		  $result = $conn->prepare("SELECT * FROM `tbl_fs_transactions` where fs_fund_sedol LIKE '".$sedol."';");
		  $result->execute();

		  // Parse returned data
		  while($row = $result->fetch(PDO::FETCH_ASSOC)) {
			  $shares += $row['fs_shares'];
		  }
...
function gimme_shares(){
	...
	try {
	  ...
	  $query = "SELECT DISTINCT fs_fund_sedol FROM `tbl_fs_transactions` WHERE bl_live = 1;";
	...
	foreach ($funds as $fund):
		...
		try {
		  ...
		  $result = $conn->prepare("SELECT * FROM `tbl_fs_transactions` where fs_fund_sedol LIKE '".$fund['fs_fund_sedol']."';");
		  $result->execute();

		  // Parse returned data
		  while($row = $result->fetch(PDO::FETCH_ASSOC)) {
			  $sedol = $row['fs_fund_sedol'];
			  $shares[$sedol] += $row['fs_shares'];
		  }
...
function getFields($tbl,$srch,$param,$condition = '='){      #  "SELECT * FROM $tbl WHERE $srch $condition '$param' AND bl_live > 0;"
	global $host,$user, $pass, $db, $charset;
	try {
	  // Connect and create the PDO object
	  $conn = new PDO("mysql:host=$host; dbname=$db", $user, $pass);
...
function getField($tbl,$fld,$srch,$param){    #  "SELECT * FROM $tbl WHERE $srch = '$param';"
...
function getStratVal($assetID,$stratID,$strat){    #  "SELECT * FROM $tbl WHERE $srch = '$param';"
	...
	try {
	  ...
	  $result = $conn->prepare("SELECT $strat FROM `tbl_fs_asset_strat_vals` WHERE asset_id = '$assetID' AND strat_id = '$stratID' AND bl_live = '1';");
	  $result->execute();
...
function getStratTheme($themeID,$stratID,$strat){    #  "SELECT * FROM $tbl WHERE $srch = '$param';"
	...
	try {
	  ...
	  $result = $conn->prepare("SELECT $strat FROM `tbl_fs_theme_strats` WHERE theme_id = '$themeID' AND strat_id = '$stratID' AND bl_live = '1';");
	  $result->execute();
