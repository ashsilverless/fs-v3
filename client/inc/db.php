...
function getLastDate($tbl,$retfield,$orderFld = "id",$param){
	...
	try {
	  ...
	  $result = $conn->prepare("SELECT $retfield FROM $tbl WHERE $param AND bl_live = 1 ORDER BY $orderFld DESC LIMIT 1; ");
	  $result->execute();
...
function get_current_price($code, $enquiry_date=''){
    global $host,$user, $pass, $db, $charset;
	...
	if($enquiry_date == ''){ $enquiry_date = $today; };
	...
	try {
	  ...
	  $conn->exec("SET CHARACTER SET $charset");      // Sets encoding UTF-8
        ...
        $query = "SELECT * FROM tbl_fs_fund WHERE isin_code LIKE '$code' AND correct_at <= '$enquiry_date' AND bl_live = 1 ORDER BY correct_at DESC LIMIT 1;";
